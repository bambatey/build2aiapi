...
class StructuralChatResponse(BaseModel):
    """Yapısal mühendislik chat yanıtı"""
    answer: str = Field(description="Kullanıcının sorusuna detaylı yanıt. Türkçe, markdown formatında.")
...
class StructuralChatSignature(dspy.Signature):
    ...
    user_question: str = dspy.InputField(description="Kullanıcının sorusu")
    file_content: str = dspy.InputField(description=".s2k dosya içeriği")

    response: StructuralChatResponse = dspy.OutputField(description="Yanıt")
