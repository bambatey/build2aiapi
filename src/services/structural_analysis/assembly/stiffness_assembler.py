...
from ..elements import FrameElement3D, FrameKernel, build_shell
...
def assemble_stiffness(
    ...
    ``frame_kernels`` verilirse her frame için K_local/T pre-computed
    cache'ten okunur; verilmezse eski pattern (her frame için
    FrameElement3D oluştur) fallback olarak çalışır.
    """
    rows: list[int] = []
    cols: list[int] = []
    data: list[float] = []
    ...
    for el_id, el_dto in model.frame_elements.items():
        kernel = frame_kernels.get(el_id) if frame_kernels else None
        ...
        if kernel is None:
            ...
            ni, nj = el_dto.nodes
            # K_global = T_inv @ K_local_released @ T — cache'ten zero-cost okuma
            K_e = kernel.T_inv @ kernel.K_local_released @ kernel.T
